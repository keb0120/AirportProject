...
//				if(conn != null) conn.close();
	...
	public String[] getContinentData() {
		...
		int index=0;
		StringBuffer sql = new StringBuffer();
		sql.append("select DISTINCT 지역 from airportInfo order by 지역 asc");
		try {
			pstmt = conn.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
			while(rs.next()) { 
			...
			// TODO Auto-generated catch block
			System.out.println( " getContinentdata" + "오류" + e.getStackTrace());
		}finally {
			try {
//				pstmt.clearParameters();
				if(rs != null) rs.close();
...
//				if(conn != null) conn.close();
	...
	public String[] getData(String target) {
		...
		int index=0;
		StringBuffer sql = new StringBuffer();
		sql.append("select DISTINCT 한글국가명 from airportInfo where 지역 =\'"+target+"\' order by 한글국가명 asc");
		try {
			pstmt = conn.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
			while(rs.next()) { 
